...
local function push_pill(out, text, opts)
  ...
  -- Spacer between pills, reset to status background
...
local function workspace_display_list(names, active, max_items)
  ...
  -- Always show the default workspace (if present) and the active one
...
wezterm.on("update-status", function(window, pane)
  ...
  -- A tiny, low-noise hint line (keyboard + mouse)
  push_text(right, "⌘⇧P workspaces  ·  ⌘⇧O tabs  ·  ⌘W close tab  ·  +Right workspaces  ")
  window:set_right_status(wezterm.format(right))
end)

-- Turn the + button into a workspace launcher on right click.
-- Keeps left click as "new tab" (default action) to avoid surprise.
wezterm.on("new-tab-button-click", function(window, pane, button, default_action)
  ...
  -- Optional: middle click opens tab navigator (mouse-friendly tab switcher)
  if button == "Middle" then
    window:perform_action(act.ShowTabNavigator, target_pane)
    return false
  end

  -- Left click -> default (+ creates a new tab)
